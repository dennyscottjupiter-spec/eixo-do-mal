...
function renderDetail(){
  ...
  h+=`<div class="kv" title="Elite amphibious troops — attack power 3, defense 3 each. Marine assaults bypass half the defender's turrets and bunker fortifications. Oil upkeep 0.5/unit/turn. Use the MARINE ASSAULT button in the ATTACK tab."><span>${CONFIG.units.marine.i} Marines <span class="dim">(amphibious)</span></span><span>${fmt(n.army.marine)}</span></div>`;
...
function renderTabContent(){
  ...
  if(UI.tab==='build'){
    ...
    const UW={infantry:'Cheap front-line soldiers. Good in large numbers for both attack and defense. No oil upkeep.',tank:'4× attack power. Heavy armor for breakthroughs — needs oil or fights at 50% strength.',jet:'6× attack power and intercepts enemy SCUDs. Powers the AIR STRIKE mode. Oil-hungry but deadly.',spy:'Enables covert ops: steal gold, blow up buildings, poison population, gather intel. More spies = higher success rate.',turret:'Static defense tower — holds the line at no oil or food cost. Cannot attack, only defend.',marine:'Elite amphibious troops (atk 3, def 3). Marine Assault bypasses half of enemy turrets and bunker fortifications — great vs heavily-fortified nations.',scud:'One-use missile: destroys 1-2 enemy buildings from a distance. Enemy jets may intercept it.'};
    ...
    if(!t||!t.alive){
      ...
      h+=`<div class="row"><span class="grow" title="Ground invasion: commit infantry + tanks. Win → seize land + loot gold + raze buildings. Lose → heavy troop casualties. Costs 2 actions.">⚔️ GROUND INVASION <span class="dim">(infantry+tanks · ▸2 actions · own: ${fmt(me.army.infantry)} inf, ${fmt(me.army.tank)} tanks)</span></span>`+
         [25,50,75,100].map(p=>`<button class="btn warn" data-a="attack" data-p="${p}" data-q="ground" title="Commit ${p}% of infantry+tanks. Win → loot gold + seize land. Lose → heavy casualties. Costs 2 actions." ${G.actions<2?'disabled':''}>${p}%</button>`).join('')+`</div>`;
      h+=`<div class="row"><span class="grow" title="Air strike: jets bomb enemy buildings (2–4 razed on hit). No land seized — jets risk interception by enemy jets. Costs 2 actions.">✈️ AIR STRIKE <span class="dim">(jets only · ▸2 actions · own: ${fmt(me.army.jet)} jets)</span></span>`+
         [50,100].map(p=>`<button class="btn warn" data-a="attack" data-p="${p}" data-q="air" title="Commit ${p}% of your jets in a bombing run — razes 2–4 buildings on success. Jets risk interception. Costs 2 actions." ${(G.actions<2||me.army.jet<1)?'disabled':''}>${p}%</button>`).join('')+`</div>`;
      h+=`<div class="row"><span class="grow" title="Marine assault: amphibious landing bypasses turrets and bunker fortifications. Seizes land + loots gold. Costs 2 actions.">⚓ MARINE ASSAULT <span class="dim">(marines only · ▸2 actions · own: ${fmt(me.army.marine)} marines)</span></span>`+
         [50,100].map(p=>`<button class="btn warn" data-a="attack" data-p="${p}" data-q="marine" title="Commit ${p}% of your marines in an amphibious assault — bypasses turrets and bunkers. Costs 2 actions." ${(G.actions<2||me.army.marine<1)?'disabled':''}>${p}%</button>`).join('')+`</div>`;
